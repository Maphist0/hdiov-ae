...
echo "RSA CRT Decrypt 1K: $(pcregrep -M -A 6 'RSA.*\n.*1024$' $INPUT | grep Operations | awk '{ print $4 }') ops/s"
echo "RSA CRT Decrypt 2K: $(pcregrep -M -A 6 'RSA.*\n.*2048$' $INPUT | grep Operations | awk '{ print $4 }') ops/s"
echo "RSA CRT Decrypt 4K: $(pcregrep -M -A 6 'RSA.*\n.*4096$' $INPUT | grep Operations | awk '{ print $4 }') ops/s"
...
echo "ECDSA          384: $(pcregrep -M -A 8 'ECDSA.*\n.*384$' $INPUT | grep Operations | awk '{ print $4 }') ops/s"
